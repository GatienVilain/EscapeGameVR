...
public class MusicsManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //Au d�but du jeu ou si la musique s'arr�te pour une raison ou pour une autre (cela ne devrait pas arriver)
            PlayNextMusic();
            scene = currentScene;
        }
        ...
        {
            //Si on veut passer � la prochaine musique
            stopTransition = true;
            stopWaitForEnd = true;
            PlayNextMusic();
            playNextMusic = false;
    ...
    private IEnumerator SoftTransition(AudioClip newMusic)
    {
        ...
        //Si on relance la musique parce que la scene a chang�, on n'arr�te pas cette transition
        ...
        while (fadingTrack.volume > 0)
        {
            ...
            {
                //Si la scene change alors qu'on est dans cette boucle, alors on arr�te la coroutine
                stopTransition = false;
                yield break;
            }
        }
